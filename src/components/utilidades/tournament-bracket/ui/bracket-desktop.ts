...
export class DesktopBracketRenderer {
    ...
    public render(data: TournamentData) {
        ...
        rounds.forEach((round, rIndex) => {
            ...
            header.className =
                "text-center font-bold text-slate-400 mb-6 uppercase tracking-wider text-xs border-b border-slate-200 pb-2 bg-slate-50/80 backdrop-blur sticky top-0 z-30";
    ...
    private getMatchCardHTML(match: Match, scoreEnabled?: boolean): string {
        ...
        const p1Class =
            winnerId && winnerId === p1?.id ? THEME.match.winner : p1 ? THEME.match.hover : "";
        const p2Class =
