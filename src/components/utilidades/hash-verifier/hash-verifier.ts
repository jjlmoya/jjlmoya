export class SHA256 {
    ...
    private transform(block: Uint8Array) {
        ...
        for (let i = 16; i < 64; i++) {
            const s0 =
                SHA256.rotateRight(w[i - 15], 7) ^
                SHA256.rotateRight(w[i - 15], 18) ^
                (w[i - 15] >>> 3);
            const s1 =
                SHA256.rotateRight(w[i - 2], 17) ^
                SHA256.rotateRight(w[i - 2], 19) ^
                (w[i - 2] >>> 10);
        ...
        for (let i = 0; i < 64; i++) {
            const S1 =
                SHA256.rotateRight(e, 6) ^ SHA256.rotateRight(e, 11) ^ SHA256.rotateRight(e, 25);
            const ch = (e & f) ^ (~e & g);
            const temp1 = (h + S1 + ch + SHA256.K[i] + w[i]) | 0;
            const S0 =
                SHA256.rotateRight(a, 2) ^ SHA256.rotateRight(a, 13) ^ SHA256.rotateRight(a, 22);
