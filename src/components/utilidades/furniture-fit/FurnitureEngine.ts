    ...
    static calculateFit(object: Dimensions, container: Dimensions, margin: number = 2): FitResult {
        ...
        if (objDims[0] <= conDims[0] && objDims[1] <= conDims[1] && objDims[2] <= conDims[2]) {
            return {
                ...
                clearance: Math.min(conDims[0] - objDims[0], conDims[1] - objDims[1], conDims[2] - objDims[2]),
                maxDiagonal: 0
            };
        }
        ...
        if (objMaxLength <= con3DDiagonal && otherTwoObj[0] <= otherTwoCon[0] && otherTwoObj[1] <= otherTwoCon[1]) {
            return {
                fits: true,
