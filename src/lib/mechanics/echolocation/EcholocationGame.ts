...
export class EcholocationGame {
    ...
    initLevel() {
        ...
        // Borders (Stone)
        this.platforms.push({ x: 0, y: 0, w: w, h: 20, material: 'stone' }); // Top
        this.platforms.push({ x: 0, y: h - 20, w: w, h: 20, material: 'stone' }); // Bottom
        this.platforms.push({ x: 0, y: 0, w: 20, h: h, material: 'stone' }); // Left
        this.platforms.push({ x: w - 20, y: 0, w: 20, h: h, material: 'stone' }); // Right

        // Platforms
        this.platforms.push({ x: 100, y: h - 100, w: 200, h: 20, material: 'wood' });
        this.platforms.push({ x: 400, y: h - 200, w: 150, h: 20, material: 'metal' }); // Bouncy
        this.platforms.push({ x: 150, y: h - 300, w: 150, h: 20, material: 'cloth' }); // Absorbing
        this.platforms.push({ x: 500, y: h - 400, w: 20, h: 100, material: 'stone' }); // Wall
    ...
    emitSound(x: number, y: number, radius: number, rayCount: number, type: 'step' | 'jump' | 'land' | 'shout' | 'bump' = 'step') {
        // Audio Feedback
        ...
        // Exaggerate ray counts and radius for better visibility
    ...
    checkCollision(axis: 'x' | 'y') {
        for (const plat of this.platforms) {
            if (this.rectIntersect(this.player.x, this.player.y, this.player.width, this.player.height, plat.x, plat.y, plat.w, plat.h)) {
                if (axis === 'x') {
                    if (this.player.vx > 0) {
                        this.player.x = plat.x - this.player.width;
                        if (Math.abs(this.player.vx) > 1) this.emitSound(this.player.x + 20, this.player.y + 10, 120, 60, 'bump');
                    } else {
                        this.player.x = plat.x + plat.w;
                        if (Math.abs(this.player.vx) > 1) this.emitSound(this.player.x, this.player.y + 10, 120, 60, 'bump');
                    }
                    this.player.vx = 0;
                } else {
                    if (this.player.vy > 0) {
                        ...
                        // Land sound
                        if (this.player.vy > 2) this.emitSound(this.player.x + 10, this.player.y + 20, 150, 100, 'land');
                        this.player.vy = 0;
                    } else {
    ...
    rectIntersect(x1: number, y1: number, w1: number, h1: number, x2: number, y2: number, w2: number, h2: number) {
