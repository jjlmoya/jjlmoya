...
export class Renderer {
    ...
    private drawParticles(particles: Particle[], velocity: number) {
        const warpFactor = Math.max(0, (velocity - CONSTANTS.WARP_SPEED) / (CONSTANTS.MAX_SPEED - CONSTANTS.WARP_SPEED));
        particles.forEach(p => {
            const streak = Math.max(2, velocity * (0.1 + warpFactor * 0.5));
            this.ctx.fillStyle = warpFactor > 0.5 ? `rgba(255, 255, 255, ${p.alpha})` : `rgba(14, 165, 233, ${p.alpha * 0.6})`;
    ...
    private drawScrap(scrap: Scrap[], time: number) {
        scrap.forEach(s => {
            if (!s.gathered) {
                ...
                this.ctx.fillRect(-s.size / 2 * pulse, -s.size / 2 * pulse, s.size * pulse, s.size * pulse);

                this.ctx.strokeStyle = '#fff';
                this.ctx.lineWidth = 2;
                this.ctx.strokeRect(-s.size / 2 * pulse, -s.size / 2 * pulse, s.size * pulse, s.size * pulse);
                this.ctx.restore();
            }
        });
    }
    ...
    private drawObstacles(obstacles: Entity[], time: number) {
        obstacles.forEach(o => {
            ...
            for (let n = 0; n < 8; n++) {
                ...
                if (n === 0) this.ctx.moveTo(px, py); else this.ctx.lineTo(px, py);
            }
            this.ctx.closePath();
            this.ctx.fill();
    ...
    private drawStats(score: number, upgrades: Upgrades, isAtBase: boolean) {
        ...
        if (isAtBase) {
            ...
            this.ctx.fillText(`1. MOTOR (Nivel ${upgrades.engine}) - Coste: ${CONSTANTS.UPGRADE_COST}`, 0, 180);
            this.ctx.fillText(`2. CASCO (Nivel ${upgrades.hull}) - Coste: ${CONSTANTS.UPGRADE_COST}`, 0, 200);
            this.ctx.fillText(`3. MANIOBRA (Nivel ${upgrades.steering}) - Coste: ${CONSTANTS.UPGRADE_COST}`, 0, 220);
        }
    }

    private drawPointers(scrap: Scrap[], playerX: number, playerY: number, playerAngle: number, time: number) {
    ...
    public drawVignette() {
        const g = this.ctx.createRadialGradient(this.canvas.width / 2, this.canvas.height / 2, 0, this.canvas.width / 2, this.canvas.height / 2, Math.max(this.canvas.width, this.canvas.height) * 0.6);
