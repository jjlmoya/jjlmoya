...
export class AISystem {
    public static update(enemy: Entity, player: Entity | undefined, addBullet: (b: Entity) => void): void {
    ...
    private static shoot(enemy: Entity, player: Entity, addBullet: (b: Entity) => void): void {
        ...
        addBullet({
            ...
            position: { x: enemy.position.x + enemy.size / 2, y: enemy.position.y + enemy.size / 2 },
            velocity,
            size: BULLET_SIZE,
            type: 'enemy_bullet',
            destroyed: false
