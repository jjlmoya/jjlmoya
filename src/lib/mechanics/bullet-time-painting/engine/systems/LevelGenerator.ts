    ...
    public static generateObstacles(level: number): Obstacle[] {
        ...
        for (let i = 0; i < count; i++) {
            ...
            do {
                ...
                obs = {
                    x: STAGE_PADDING + 100 + Math.random() * (CANVAS_WIDTH - STAGE_PADDING * 2 - width - 200),
                    y: STAGE_PADDING + 100 + Math.random() * (CANVAS_HEIGHT - STAGE_PADDING * 2 - height - 200),
                    width,
                    height
                };

                
                overlapping = obstacles.some(other =>
                ...
                if (Math.abs(obs.x + obs.width / 2 - centerX) < 200 && Math.abs(obs.y + obs.height / 2 - centerY) < 200) {
                    overlapping = true;
                }
