...
export class Renderer {
    ...
    private drawFloor(): void {
        ...
        this.ctx.strokeRect(STAGE_PADDING, STAGE_PADDING, CANVAS_WIDTH - STAGE_PADDING * 2, CANVAS_HEIGHT - STAGE_PADDING * 2);
    }
    ...
    private drawEntities(entities: Entity[]): void {
        entities.forEach(entity => {
            this.ctx.save();
            this.ctx.translate(entity.position.x + entity.size / 2, entity.position.y + entity.size / 2);
            ...
            this.ctx.strokeRect(entity.size / 4, -entity.size / 6, entity.size / 2, entity.size / 3);
            this.ctx.restore();
        });
    }
    ...
    private drawPlayer(player: Entity | undefined): void {
        if (!player) return;
        this.ctx.save();
        this.ctx.translate(player.position.x + player.size / 2, player.position.y + player.size / 2);
    ...
    private drawArrow(origin: Vector2D, vector: Vector2D, type: 'move' | 'shoot', persistent: boolean): void {
        let endX: number;
        let endY: number;
