...
export class DrawThePathGame {
    ...
    private draw() {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            this.ctx.beginPath();
            this.ctx.arc(pl.x - pl.vx * i * 1.5, pl.y - pl.vy * i * 1.5, pl.radius * (1 - i * 0.1), 0, Math.PI * 2);
            this.ctx.fill();
        }
    ...
    private bindInput() {
        ...
        const start = (e: MouseEvent | TouchEvent) => {
            ...
            const t = (window.TouchEvent && e instanceof TouchEvent) ? e.touches[0] : (e as MouseEvent);
            const p = getPos(t);
