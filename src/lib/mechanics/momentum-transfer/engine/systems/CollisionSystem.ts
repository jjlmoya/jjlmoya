...
export class CollisionSystem {
    ...
    private static resolveWallCollision(entity: Entity, wall: Obstacle) {
        ...
        if (dx === 0 && dy === 0) {
            ...
            if (min === dLeft) { entity.pos.x = wall.x - entity.radius; entity.vel.x *= -1; }
            else if (min === dRight) { entity.pos.x = wall.x + wall.width + entity.radius; entity.vel.x *= -1; }
            else if (min === dTop) { entity.pos.y = wall.y - entity.radius; entity.vel.y *= -1; }
            else { entity.pos.y = wall.y + wall.height + entity.radius; entity.vel.y *= -1; }
            return;
        }
