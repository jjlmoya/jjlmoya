...
export class BatteryLifeEstimator {
    public static calculate(specs: BatterySpecs): SoHResult {
        ...
        if (specs.voltage < 3.20) voltageStress = 1.2; 
        ...
        const estimatedLife = Math.max(0, baseYears * remainingLifeRatio * (1 / (tempPenalty * voltageStress)));
    ...
    private static getRecommendations(specs: BatterySpecs, soh: number): string[] {
        const recs: string[] = [];
        if (specs.temperature > 35) recs.push('Reduce la temperatura ambiente o mejora la ventilación para evitar la oxidación del electrolito.');
        if (specs.voltage > 4.1) recs.push('Evita mantener la batería al 100% de carga (4.2V) por periodos prolongados.');
        if (specs.voltage < 3.3) recs.push('Evita descargas profundas; los ciclos entre 20% y 80% duplican la vida útil.');
        if (soh < 80) recs.push('La capacidad ha caído por debajo del estándar óptimo. Considera un reemplazo si la autonomía es insuficiente.');
        if (recs.length === 0) recs.push('Mantén los hábitos actuales, tu batería está en un rango de operación ideal.');
        return recs;
    }
}
