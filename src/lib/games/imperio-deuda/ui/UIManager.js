...
export class UIManager {
    ...
    initEvents() {
        ...
        viewport.addEventListener("click", (e) => {
            ...
            if (!this.hasMovedWhilePanning && !this.draggedBizId && !e.target.closest('.zoom-btn') && !e.target.closest('.building-info-modal')) {
                this.handleClick(e);
            }
        });
    ...
    showBuildingDetails(gx, gy, data) {
        ...
        const synergyInfo = b.proximityMult > 1 ? `<div style="margin-top:12px; padding:10px; background:rgba(16, 185, 129, 0.1); border-radius:12px; font-size:11px; color:var(--success); font-weight:800; display:flex; align-items:center; gap:8px;">🚀 SINERGIA ACTIVA: x${b.proximityMult.toFixed(1)}</div>` : "";
    ...
    updateStats() {
        ...
        if (this.els.bank) this.els.bank.innerHTML = `${Format.num(this.engine.bank.value)}<span>${this.CURRENCY_SVG}</span>`;
        if (this.els.income) this.els.income.innerHTML = `+${Format.num(this.engine.getTotalIncome())}/s`;
        if (this.els.growth) this.els.growth.innerText = `+${(this.engine.debtGrowthRate * 1000).toFixed(2)}%/s`;
    ...
    renderGrid() {
        ...
        for (let y = 0; y < DIM; y++) {
            for (let x = 0; x < DIM; x++) {
                ...
                slot.ondragleave = () => { slot.classList.remove("preview-ok", "preview-error"); };
                slot.ondrop = (e) => {
                    e.preventDefault();
            ...
            if (handle) {
                ...
                handle.ontouchmove = (e) => {
                    ...
                    if (slot) {
                        ...
                        const ok = bx.allowedOn === (isRoad ? "road" : "block") && !this.engine.grid.has(`${slot.dataset.x},${slot.dataset.y}`);
                        slot.classList.add(ok ? "preview-ok" : "preview-error");
                    }
                    e.preventDefault();
                };
                ...
                handle.ontouchend = (e) => {
                    ...
                    const target = document.elementFromPoint(touch.clientX, touch.clientY);
                    const slot = target ? target.closest(".grid-slot") : null;
                    if (slot) this.engine.placeBuilding(parseInt(slot.dataset.x), parseInt(slot.dataset.y), this.draggedBizId);

                    this.touchElement.remove(); this.touchElement = null; this.draggedBizId = null;
                    this.render();
                };
            }
    ...
    renderPassives() {
        this.engine.businesses.forEach((b) => {
            const next = this.engine.passives.filter(p => p.bId === b.id).find(p => !p.purchased);
    ...
    renderIcons() {
        ...
        this.engine.passives.filter(p => p.purchased).forEach(p => {
            const d = document.createElement("div");
            d.className = "p-icon-chip"; d.style.color = "var(--accent)";
