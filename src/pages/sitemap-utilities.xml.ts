...
import { getAllRegisteredTools, buildEsSlugMap, CATEGORIES } from "../i18n/toolRegistry";
...
export const GET: APIRoute = async () => {
    ...
    for (const [tool, categoryKey] of await getAllRegisteredTools(CATEGORIES)) {
        ...
        const alternates = await getUtilityAlternates(toolDef, category.entry);
        if (alternates.length === 0) continue;

        entries.push(urlEntry(loc, alternates));
    }

    for (const esPageSlug of Object.keys(await buildEsSlugMap(CATEGORIES))) {
