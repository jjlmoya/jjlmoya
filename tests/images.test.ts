...
describe('Image Assets Integrity', () => {
    ...
    it('should only contain WebP images in public/ (except .ico and .svg)', async () => {
        ...
        for (const file of validDataFiles) {
            ...
            if (checkedExtensions.includes(ext)) {
                if (ext !== '.webp') {
                    errors.push(`File ${path.relative(PUBLIC_DIR, file)} has invalid extension '${ext}'. Should be '.webp'.`);
                } else {
                    try {
                        const metadata = await sharp(file).metadata();
                        if (metadata.format !== 'webp') {
                            errors.push(`File ${path.relative(PUBLIC_DIR, file)} has extension .webp but is actually ${metadata.format}.`);
                        }
                    } catch (err: any) {
    ...
    it('should not reference non-webp images in src/** code', () => {
        ...
        for (const file of files) {
            if (['.webp', '.ico', '.png', '.jpg'].includes(path.extname(file).toLowerCase())) continue;
            ...
            lines.forEach((line, index) => {
                ...
                forbiddenPatterns.forEach(pattern => {
                    if (pattern.test(line)) {
                        errors.push(`File ${path.relative(SRC_DIR, file)}:${index + 1} references forbidden image format: ${line.trim()}`);
                    }
                });
            });
        }
