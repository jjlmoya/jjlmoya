    ...
    pages.forEach((pagePath) => {
        ...
        it(`La página "${pagePath}" debe tener una imagen OG personalizada (no el favicon)`, async () => {
            ...
            try {
                ...
                throw new Error(`No se pudo conectar con ${BASE_URL}. ¿Está el servidor dev en marcha?`);
            }

            expect(response.status, `La página ${pagePath} devolvió un error ${response.status}`).toBe(200);
            ...
            expect(ogImageMatch, `CRÍTICO: La página "${pagePath}" NO tiene la etiqueta <meta property="og:image">`).not.toBeNull();
            expect(twitterImageMatch, `CRÍTICO: La página "${pagePath}" NO tiene la etiqueta <meta property="twitter:image">`).not.toBeNull();
            ...
            expect(ogImage.length, `La URL de la imagen OG en "${pagePath}" está vacía`).toBeGreaterThan(10);
        });
    });
});
